...
void drawItems(){
  ...
  if(g_vert){
    ...
    line(i, 0, i, height);
    textAlign(LEFT);
    if (g_values && i%(2*gridsize)==0 && i < (width-(width/10))) { fill((bck) ? (0) : (255) );  text( empty+i, (i+fontsize/4), 0+fontsize);
                                                                   fill((bck) ? (232) : (32) ); text( empty+i, (i+fontsize/4), 0+fontsize);
                                                                 }
  }}
  ...
  if(g_horiz){
    ...
    line(0, i, width, i);
    textAlign(LEFT); 
    if (g_values && i%(2*gridsize)==0) { fill((bck) ? (0) : (255) ); text( empty+(height-i), 0+(fontsize/4), i-(fontsize/4));
                                         fill((bck) ? (232) : (32) ); text( empty+(height-i), 0+(fontsize/4), i-(fontsize/4));
                                         }
        }}
  }
 }
